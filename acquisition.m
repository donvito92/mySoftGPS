function [acqResults, results, acquiredSvs] = acquisition(signalSnap, settings)
...
signal1 = signalSnap(1 : samplesPerCode);
signal2 = signalSnap(samplesPerCode+1 : 2*samplesPerCode);
...
results     = zeros(length(settings.acqSatelliteList), numberOfFrqBins, samplesPerCode);
...
% A list of the satellites successfully acquired
acquiredSvs = [];
...
%% Correlate signals ======================================================   
    ...
    for frqBinIndex = 1:numberOfFrqBins
        ...
        if (max(acqRes1) > max(acqRes2))
            results(PRN, frqBinIndex, :) = acqRes1;
        else
            results(PRN, frqBinIndex, :) = acqRes2;
        end
...
%% Look for correlation peaks in the results ==============================
    ...
    [peakSize frequencyBinIndex] = max(max(results(PRN, :, :), [], 3));

    %--- Find code phase of the same correlation peak ---------------------
    [peakSize codePhase] = max(max(results(PRN, :, :), [], 2));
    ...
    secondPeakSize = max(results(PRN, frequencyBinIndex, codePhaseRange));
