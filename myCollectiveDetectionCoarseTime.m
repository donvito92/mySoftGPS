function [collectiveCorrelogram count] = myCollectiveDetectionCoarseTime(roughEstimate, results, eph, settings, acqResults)
            ...
            for U = roughEstimate.U - roughEstimate.uncertaintyU : roughEstimate.stepU : roughEstimate.U + roughEstimate.uncertaintyU
                ...
                for satNr = 1 : length(eph.PRN)
                    ...
                    for CCPRN = 1 : length(eph.PRN)
                        ...
                        collectiveCorrelogram(indexN, indexE, indexU, indexB, indexT) = collectiveCorrelogram(indexN, indexE, indexU, ...
                                                                                        indexB, indexT) + max(results(eph.PRN(CCPRN), :, codePhase));
